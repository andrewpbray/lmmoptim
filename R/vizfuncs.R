...
showlines <- function(lines) {
  p <- ggplot ( subset (lines, is.finite(slope) & slope < 0),
       ...
       geom_segment() +
       geom_vline ( data = subset ( lines, slope == -Inf ),
                    aes ( xintercept = int.sigsqe ),
                    lty = 2
                  ) +
       theme_bw()
...
showboxes <- function(boxes) {
  p <- ggplot ( boxes,  aes ( xmin = sigsqe.lo,
                                xmax = sigsqe.hi,
                                ymin = sigsqs.lo,
                                ymax = sigsqs.hi
                              )
              )
  p + geom_rect ( color="black", fill="white" ) +
      xlab("Sigma_e^2") + ylab("Sigma_s^2")
...
showfunc <- function(boxes){
  ...
  return (
    p + geom_rect ( aes ( fill =  rll.lower ) ) +
	    scale_fill_continuous ( limits=c(mle.val-20,mle.val),
	                            low="black", high="white", na.value="black",
	                            name=expression(L^b)
	                          ) +
	    geom_rect ( data=boxes[hi,], aes(fill=rll.lower), color="blue" ) +
	    geom_point ( data = boxes[mle,],
	                 aes ( x = sigsqe.lo, y = sigsqs.lo ),
	                 color="red", size=5
	               )
  )
}
