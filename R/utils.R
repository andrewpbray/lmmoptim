...
RLL <- function ( sigsqe, sigsqs ) {
	...
	return ( sum ( with ( lines, -.5 * ( multiplier.log*log(terms.lin) + multiplier.inv/terms.lin ) ) ) )
}
...
RLL2 <- function ( x, y, z, SigE, SigS, sigsqe, sigsqs ) {
	...
	-.5 * ( det(V,log=TRUE) + det ( xVinv %*% x, log=TRUE ) +
	           t(y) %*% ( Vinv - t(xVinv) %*% solve ( xVinv %*% x ) %*% xVinv ) %*% y
	        )
}

whichbox <- function ( sigsqe, sigsqs, boxes ) {
	# Which box contains ( sigsqe, sigsqs )
	tmp <- t ( sapply ( X = boxes,
	                              FUN = function(box) { c ( box$lims.sigsqe, box$lims.sigsqs ) }
	                            )
	              )
	return ( which ( sigsqe > tmp[,1] & sigsqe < tmp[,2] & sigsqs > tmp[,3] & sigsqs < tmp[,4] ) )
}