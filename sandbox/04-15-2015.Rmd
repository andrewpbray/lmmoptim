...
## 
```{r, echo=FALSE, fig.width = 3, message = FALSE}
library(wesanderson)
library(dplyr)
...
inputPanel(
  numericInput("eps", label = "epsilon", value = .1, min = .01, max = .5, step = .1),
  sliderInput("it", label = "iteration", min = 1, max = 24, value = 1, step = 1,
              animate = animationOptions(loop = F, interval=1500)),
  radioButtons("radio", label = "stage",
    choices = list("branch" = 1, "bound" = 2, "kill" = 3),
    selected = 2)
)
...
renderPlot({
  ...
  for (i in segs) { # bound
    ...
    if (input$it %in% seq(2, 24, 3)) { # new kill
      updateRadioButtons(session, "radio", selected = 3)
